...
class ImprovedDonchianStrategy:
        ...
        try:
            ...
            if action == "SELL":
                if actual_position <= 0:
                    self.logger.warning("没有可卖出的仓位，取消交易")
                    return False
                max_sell = actual_position
            else:  # 买入
                # 计算最大可买数量（考虑资金限制）
                max_buy = int(self.max_capital_per_trade / price)
            
            # 计算本次交易量
            position_size = (
                min(actual_position, self.calculate_position_size(price))
                if action == "SELL"
                else min(max_buy, self.calculate_position_size(price))
            )
            ...
            # 执行交易
            total_filled = 0
            for i in range(self.base_tranches):
                # 每次分批前都重新验证仓位
                self.verify_position()
                
                # 计算本批次数量
                batch_size = position_size // self.base_tranches
                if action == "SELL":
                    # 确保不会超卖
                    remaining_position = self.verify_position()
                    batch_size = min(batch_size, remaining_position - total_filled)
                    
                if batch_size < 1:
                    self.logger.info("批次数量不足，停止交易")
                    break
                    
                filled = self.execute_tranche(action, batch_size, price)
                if filled:
                    total_filled += filled
                ...
                if i < self.base_tranches - 1 and filled:
                    time.sleep(20)
    ...
    def update_position_info(self, action, filled_quantity, price):
        ...
        try:
            ...
            if action == "BUY":
                self.current_position_size = actual_position + filled_quantity
                self.total_cost_basis += filled_quantity * price
                self.logger.info(f"买入更新 - 数量: {filled_quantity}, 价格: ${price:.2f}")
            else:  # SELL
                if actual_position > 0:
                    avg_cost = self.total_cost_basis / actual_position
                    ...
                    if self.current_position_size > 0:
                        ...
                        self.total_cost_basis = 0
                    self.logger.info(f"卖出更新 - 数量: {filled_quantity}, 价格: ${price:.2f}, 实现盈亏: ${realized_pnl:.2f}")
